...
public:
    ...
    inline void goLeft(const float step=0.01f) {position += -VM::cross(up, direction) * step;}
    inline void goRight(const float step=0.01f) {position -= -VM::cross(up, direction) * step;}
